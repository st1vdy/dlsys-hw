...
__device__ size_t index_map(size_t gid, CudaVec& shape, CudaVec& strides, size_t offset) {
  auto ndim = shape.size;
  auto res = offset;
  for (int i = ndim - 1; i >= 0; i--) {
    int cur_idx = gid % shape.data[i];
    gid /= shape.data[i];
    res += cur_idx * strides.data[i];
  }
  return res;
}
...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    out[gid] = a[index_map(gid, shape, strides, offset)];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    out[index_map(gid, shape, strides, offset)] = a[gid];
  }
}
...
__global__ void ScalarSetitemKernel(scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    out[index_map(gid, shape, strides, offset)] = val;
  }
}
...
// Elementwise and scalar operations
////////////////////////////////////////////////////////////////////////////////
__global__ void MatMulNaiveKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, uint32_t M, uint32_t N, uint32_t P, uint32_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    int y = gid % P, x = gid / P;
    scalar_t v = 0;
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(out->size);
  MatMulNaiveKernel<<<dim.grid, dim.block>>>(a.ptr, b.ptr, out->ptr, M, N, P, out->size);
  /// END SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) {
    auto max_val = a[gid * reduce_size];
